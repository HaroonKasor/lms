...
// SkillUp xAPI transport guard v3 (final null-score sanitizer)
(function () {
  if (window.__skillupXapiTransportGuardV3) return;
  window.__skillupXapiTransportGuardV3 = true;

  function cleanStmt(stmt) {
    if (!stmt || !stmt.result || !stmt.result.score) return;
    var s = stmt.result.score;
    ["raw","scaled","min","max"].forEach(function (k) {
      var v = s[k];
      if (v === null || v === undefined || v === "") { delete s[k]; return; }
      if (typeof v !== "number") {
        var n = Number(v);
        if (Number.isFinite(n)) s[k] = n; else delete s[k];
      }
    });
    if (Object.keys(s).length === 0) delete stmt.result.score;
  }

  function cleanPayload(obj) {
    try {
      if (Array.isArray(obj)) { obj.forEach(cleanStmt); return obj; }
      if (obj && Array.isArray(obj.statements)) { obj.statements.forEach(cleanStmt); return obj; }
      cleanStmt(obj); return obj;
    } catch (e) { return obj; }
  }
  ...
  XMLHttpRequest.prototype.open = function (m, u) {
    this.__xapiUrl = String(u || "");
    return _open.apply(this, arguments);
  };
  XMLHttpRequest.prototype.send = function (body) {
    try {
      if (this.__xapiUrl && /\/statements(\?|$)/.test(this.__xapiUrl) && typeof body === "string") {
        var obj = JSON.parse(body);
        body = JSON.stringify(cleanPayload(obj));
      }
    } catch (e) {}
  ...
  if (window.fetch) {
    ...
    window.fetch = function (input, init) {
      try {
        ...
        if (/\/statements(\?|$)/.test(url) && init && typeof init.body === "string") {
          var obj = JSON.parse(init.body);
          init.body = JSON.stringify(cleanPayload(obj));
        }
      } catch (e) {}
...
// SkillUp xAPI score guard v2
(function () {
  if (window.__skillupLrsScoreGuardV2) return;
  window.__skillupLrsScoreGuardV2 = true;

  function cleanScore(stmt) {
    try {
      if (!stmt || !stmt.result || !stmt.result.score) return;
      var s = stmt.result.score;
      ["raw","scaled","min","max"].forEach(function (k) {
  ...
  try {
    var p = TinCan && TinCan.LRS && TinCan.LRS.prototype;
    if (!p) return;

    if (p.saveStatement) {
      var _saveStatement = p.saveStatement;
      p.saveStatement = function (stmt, cfg) {
        cleanScore(stmt);
        return _saveStatement.call(this, stmt, cfg);
      };
    }

    if (p.saveStatements) {
      var _saveStatements = p.saveStatements;
      p.saveStatements = function (stmts, cfg) {
        try { if (Array.isArray(stmts)) stmts.forEach(cleanScore); } catch (e) {}
        return _saveStatements.call(this, stmts, cfg);
      };
    }
  } catch (e) {}
})();
...
// SkillUp global xAPI score sanitizer (LRS layer)
(function () {
    if (window.__skillupLrsSaveStatementPatched) return;
    window.__skillupLrsSaveStatementPatched = true;
    try {
        var proto = TinCan && TinCan.LRS && TinCan.LRS.prototype;
        if (!proto || !proto.saveStatement) return;
        var _origSaveStatement = proto.saveStatement;
        proto.saveStatement = function (stmt, cfg) {
            try {
                if (stmt && stmt.result && stmt.result.score) {
                    var s = stmt.result.score;
                    ["raw","scaled","min","max"].forEach(function (k) {
                        var v = s[k];
                        if (v === null || v === undefined || v === "") { delete s[k]; return; }
            ...
            return _origSaveStatement.call(this, stmt, cfg);
        };
    } catch (e) {}
})();
...
//TinCan.enableDebug();
        ...
        //
			...
			// SkillUp: sanitize invalid score values before sending xAPI statements.
			if (tincan && !tincan.__skillupScoreSanitized) {
				tincan.__skillupScoreSanitized = true;
				var _skillupOrigSendStatement = tincan.sendStatement;
				tincan.sendStatement = function (stmt, callback) {
					try {
						if (stmt && stmt.result && stmt.result.score) {
							var score = stmt.result.score;
							var scoreKeys = ["raw", "scaled", "min", "max"];
							for (var si = 0; si < scoreKeys.length; si++) {
								var key = scoreKeys[si];
								var value = score[key];
								...
								if (typeof value !== "number") {
									...
									if (Number.isFinite(parsed)) score[key] = parsed;
									else delete score[key];
								}
							}
							if (Object.keys(score).length === 0) delete stmt.result.score;
							if (
								stmt.result &&
								!stmt.result.score &&
								stmt.result.success == null &&
								stmt.result.completion == null &&
								stmt.result.response == null &&
								stmt.result.duration == null &&
								stmt.result.extensions == null
							) {
								delete stmt.result;
							}
						}
					} catch (e) {}
					return _skillupOrigSendStatement.call(tincan, stmt, callback);
				};
			}
		...
		if(!isoffline){
			var grouping = (
				tincan &&
				tincan.context &&
				tincan.context.contextActivities &&
				Array.isArray(tincan.context.contextActivities.grouping)
			) ? tincan.context.contextActivities.grouping : [];
			if(grouping.length > 0){
				for(var i = 0; i < grouping.length; i++){
					if(grouping[i] && typeof(grouping[i].id) == "string"){
						grouping[i].id = grouping[i].id.replace(/#/g, "");
					}
					//console.log(grouping[i] && grouping[i].id);
				}
			}
        ...
        function doStart(){
			...
			intervalTimeID = setInterval(function(){
				++totalTime;
				if(typeof(currentState.dataIndex[currentPage]) == "undefined" || currentState.dataIndex[currentPage] === null){
					currentState.dataIndex[currentPage] = {pageIndex : currentPage, id : (pageArray[currentPage] ? pageArray[currentPage].id : currentPage), attempted : false, numberattempted : 0, numberquizzed : 0, totalTime : 0, score : [], passed : false, quizzed : false};
				}
        ...
        function goToPage(page){
			...
			if(chkconfirm){
				...
				if(isoffline && typeof(pageArray[currentPage].urlOffline) != "undefined"){
					tc_argStr = (pageArray[currentPage].urlOffline.indexOf("?") != -1) ? "&" + location.search.slice(1) : location.search;
				}
				//navigate the iFrame to the content
				theIframe.src = pageArray[currentPage].url + tc_argStr;
				if(isoffline && typeof(pageArray[currentPage].urlOffline) != "undefined"){
					theIframe.src = pageArray[currentPage].urlOffline + tc_argStr;
				}
				...
				if(newData){
					...
					if(typeof(currentState.dataIndex[currentPage]) == "undefined" || currentState.dataIndex[currentPage] === null){
						currentState.dataIndex[currentPage] = {pageIndex : currentPage, id : pageArray[currentPage].id, attempted : false, numberattempted : 0, numberquizzed : 0, totalTime : 0, score : [], passed : false, quizzed : false};
					}
		...
		function updateStatus(){
			...
			if(isNaN(indexp) || indexp < 0 || indexp >= currentState.dataIndex.length){
				indexp = parseInt(currentPage);
			}
			if(isNaN(indexp) || indexp < 0 || indexp >= currentState.dataIndex.length){
				return;
			}
			var stateItem = currentState.dataIndex[indexp];
			if(typeof(stateItem) == "undefined" || stateItem === null){
				return;
			}
			var txtqtip = "ชื่อ : ";
			for(var i = 0; i < treeArray.length; i++){
			...
			else if(stateItem.attempted && stateItem.pageIndex == currentPage){
				txtqtip += "กำลังเรียน";
			}
			else if(stateItem.attempted && !stateItem.passed){
				txtqtip += "ไม่ผ่าน";
			}
			else{
				txtqtip += "ไม่เคยเข้าเรียน";
			}
			txtqtip += "<br>เข้าเรียน : " + stateItem.numberattempted+" ครั้ง";
			...
			if(pageArray[indexp].activity){
				txtqtip += "<br>ทำแบบทดสอบ : " + stateItem.numberquizzed+" ครั้ง";
				...
				if(stateItem.numberquizzed > 0 && stateItem.score.length > 0){
					txtqtip += stateItem.score[0].percent + "%";
				}
				else{
					txtqtip += "-";
				}
			...
			txtqtip += "<br>เวลา : " + getTimeString(stateItem.totalTime);
		...
		function getSessionNumber(key){
			try{
				return parsePositiveNumber(sessionStorage.getItem(key));
			}
			catch(err){
				return 0;
			}
		...
		function resolveReportContext(){
			...
			var userId = parsePositiveNumber(query.userId || query.userid || query.user_id) || getSessionNumber("currentUserId");
			var courseId = parsePositiveNumber(query.courseId || query.courseid || query.lrscourseid) || getSessionNumber("currentCourseId") || parsePositiveNumber(courseid);
			var enrollmentId = parsePositiveNumber(query.enrollmentId || query.enrollmentid || query.enrollId) || getSessionNumber("currentEnrollmentId");
			return { userId: userId, courseId: courseId, enrollmentId: enrollmentId };
		}
		...
		function markLessonEntry(){
			reportLogState.pageEnterAt = Date.now();
			reportLogState.pageEnterIndex = currentPage;
		}

		function flushLearningLog(source){
			if(reportLogState.pageEnterAt === null || reportLogState.pageEnterIndex === null){
				markLessonEntry();
				return;
			}

			var context = resolveReportContext();
			if(!context.userId || !context.courseId || !context.enrollmentId){
				markLessonEntry();
				return;
			}

			var nowMs = Date.now();
			var startedMs = reportLogState.pageEnterAt;
			var elapsedSeconds = Math.floor((nowMs - startedMs) / 1000);
			if(elapsedSeconds < 3){
				markLessonEntry();
				return;
			}

			var targetIndex = reportLogState.pageEnterIndex;
			var targetPage = pageArray[targetIndex] || {};
			var lessonTitle = stripTags(targetPage.title || targetPage.text || ("Lesson " + (targetIndex + 1)));
			var payload = {
				...
				durationMinutes: Number((elapsedSeconds / 60).toFixed(2)),
				source: source || "tincan"
			};

			if(payload.durationMinutes > 0){
				fetch("/api/report-learning/log", {
					method: "POST",
					credentials: "include",
			...
			reportLogState.pageEnterAt = nowMs;
			reportLogState.pageEnterIndex = currentPage;
		}
